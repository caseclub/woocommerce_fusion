#/apps/woocommerce_fusion/woocommerce_fusion/woocommerce
# at top of file, add:
import socket
import time
import random
...
def resolve_with_retries(host: str, attempts: int = 5, base_sleep: float = 0.4) -> None:
    """
    Try to resolve a host a few times with exponential backoff + jitter.
    ...
    for i in range(attempts):
        try:
            ...
            last_err = e
            sleep = base_sleep * (2 ** i) + random.uniform(0, 0.15)
            time.sleep(sleep)
    # surface the final error for the caller to decide
    raise last_err
    ...
    def get_list_of_records(cls, args) -> list[Union[Dict, "WooCommerceResource"]]:
        ...
        if len(wc_api_list) > 0:
            ...
            for wc_server in wc_api_list:
                ...
                try:
                    parsed = urlparse(wc_server.woocommerce_server_url)
                    host = parsed.hostname or wc_server.woocommerce_server_url
                    resolve_with_retries(host)
                except Exception as e:
                    # Log and skip this server on DNS failures
                    log_and_raise_error(
                        error_text=f"DNS preflight failed for {wc_server.woocommerce_server_url}: {str(e)}"
                    )
                    # Instead of raising, just continue to the next server
                    # (If you prefer to soft-log instead of raise, replace with frappe.log_error and continue.)
                    continue
                ...
                except (RequestsConnectionError, NameResolutionError, NewConnectionError) as err:
                    frappe.log_error(f"get_list failed for {wc_server.woocommerce_server_url}\nReason: {repr(err)}", "WooCommerce DNS/Connection Error")
                    # skip this server; try the next
                    continue
                except Exception as err:
                ...
                while True:
                    ...
                    try:
                        ...
                        frappe.log_error(f"get_list paging failed for {wc_server.woocommerce_server_url}\nReason: {repr(err)}", "WooCommerce DNS/Connection Error")
                        break  # break out of paging loop for this server                    
                    except Exception as err:
                        log_and_raise_error(err, error_text="get_list failed")
...
def get_domain_and_id_from_woocommerce_record_name(name: str) -> Tuple[str, str]:
    # Find the last occurrence of the delimiter to handle domains with '~'
    delimiter = "~"
    if delimiter not in name:
        raise ValueError(f"Invalid WooCommerce record name format: {name}")

    last_delimiter_index = name.rfind(delimiter)
    domain = name[:last_delimiter_index]
    record_id = name[last_delimiter_index + 1 :]

    return domain, record_id  # Return as string; subclasses handle parsing/int conversion
